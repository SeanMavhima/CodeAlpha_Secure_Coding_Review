...
while True:
    ...
    if time.time() - timeStart < totalTime:
        ...
        if hands and len(hands) > 0:
            # Accessing the first hand detected
            hand = hands[0]
            lmList = hand['lmList']
            bbox = hand['bbox']
            x, y, w, h = bbox
            if len(lmList) >= 18:  # Ensure enough landmarks are detected
                x1, y1 = lmList[5][:2]
                x2, y2 = lmList[17][:2]
        ...
        cvzone.putTextRect(img, 'SYSTEM IDLE', (400, 400), scale=5, offset=30, thickness=7)
        cvzone.putTextRect(img, f'SYSTEM VALUE: {score}', (450, 500), scale=3, offset=20)
